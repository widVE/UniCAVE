...
public class GrabberTool : MonoBehaviour, ITool
{
    ...
    private int rayLength = 200;
    private Vector3 origin, direction, previousOrigin, previousDirection;
    ...
    public void ButtonClick(int button, Vector3 origin_, Vector3 direction_, bool cave, bool rotate)
    {
        ...
        else
        {
            if(button == 7)
            {
                hit.transform.Rotate(new Vector3(0, -20, 0));
            }
            else if (button == 8)
            {
                hit.transform.Rotate(new Vector3(0, 20, 0));
            ...
            {
                hit.transform.Rotate(new Vector3(-20, 0, 0));
            }
            else if (button == 10)
            {
                hit.transform.Rotate(new Vector3(20, 0, 0));
