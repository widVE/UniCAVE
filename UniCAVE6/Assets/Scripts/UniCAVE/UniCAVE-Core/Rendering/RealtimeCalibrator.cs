...
namespace UniCAVE
{
    public class RealtimeCalibrator : MonoBehaviour //: NetworkBehaviour
    {
        ...
        /// </summary>
        private CalibrationType _calibrationType = CalibrationType.Vertex;
        ...
        /// </summary>
        private InfoDisplay _infoDisplayInstance;
        ...
        /// <summary>
        /// Instantiate the info screen with a delay.
        ...
        private IEnumerator InitiateInfoScreen()
        {
            ...
            if(_isServer) RpcInfoDisplayShift(selectedIndex);
        }
        ...
        private void CreateInfoDisplay()
        {
            ...
            _infoDisplayInstance = UnityEngine.Object.Instantiate(infoDisplay);
            _infoDisplayInstance.gameObject.SetActive(false);
        }
        ...
        private void LocalShift(Vector2 direction, float delta, int selectedIndex, int vertexIndex)
        {
            ...
            foreach(Dewarp dewarp in calibration.GetDisplayWarpsValues())
            {
                ...
                Dictionary<int, float> vertsToShift = GetIndexesSurrounding(dewarp.XSize, vertexIndex);
                foreach(KeyValuePair<int, float> ind in vertsToShift)
                {
        ...
        private void LocalAdjustGridSelectSize(bool increase)
        {
            ...
            {
                _gridSelectSize = (_gridSelectSize--) <= 0 ? 0 : _gridSelectSize--;
            }
        }
        ...
        {
            PhysicalDisplayCalibration currentDisplay = AllOptions[selectedIndex].Calibration;
            if(currentDisplay == null || _infoDisplayInstance == null) return;

            if (!currentDisplay.GetDisplayWarpsValues().Any()) return;
            SetInfoDisplay(_infoDisplayInstance.gameObject, currentDisplay.GetDisplayWarpsValues().First().GetDewarpGameObject().transform);
            _infoDisplayInstance.SetText(_calibrationType.ToString());
        }
        ...
        public Dictionary<int, float> GetIndexesSurrounding(int size, int index)
        {
            ...
            int indexRow = (index / (size + 1)) < 1.0f ? 0 : (int)Mathf.Floor(index / (size + 1));

            int startRow = (indexRow - _gridSelectSize) >= 0 ? (indexRow - _gridSelectSize) : 0;
            int endRow = (indexRow + _gridSelectSize) <= size ? (indexRow + _gridSelectSize) : size;

            // Vertecies to move
            Dictionary<int, float> vertsToShift = new();

            float moveFactor = (float)_gridSelectSize / (float)((_gridSelectSize * 2));
            ...
            for(int row = startRow; row <= endRow; row++)
            {
                ...
                startIndexForRow = index - (indexSizeX * (rowDiff)) - _gridSelectSize;
                startIndexForRow = (startIndexForRow < minSize) ? minSize : startIndexForRow;

                midIndexForRow = index - (indexSizeX * (rowDiff));

                stopIndexForRow = index - (indexSizeX * (rowDiff)) + _gridSelectSize;
        ...
        public void InterpolateFromCorners()
        {
            ...
            LocalInterpolateFromCorners(selectedIndex);
            if(_isServer) RpcInterpolateFromCorners(selectedIndex);
        }
        ...
        {
            _calibrationType = (from CalibrationType val in Enum.GetValues(typeof(CalibrationType)) where val > _calibrationType orderby val select val).DefaultIfEmpty().First();
            if(_isServer) RpcSetCalibrationType(_calibrationType);
            _infoDisplayInstance.SetText(_calibrationType.ToString());
        ...
        private void AdjustGridSelectSize(bool increase)
        {
            ...
            if(_isServer) RpcAdjustGridSelectSize(increase);
        }
        ...
        private void VertexShift(Vector2 direction, float delta)
        {
            ...
            if(_isServer) RpcShift(direction, delta, selectedIndex, vertexIndex);
        }
        ...
        private void PositionShift(Vector3 direction, float delta)
        {
            ...
            if(_isServer) RpcMovePosition(direction, delta, selectedIndex);
        }
        ...
        private void RotationShift(Vector3 direction, float delta)
        {
            ...
            if(_isServer) RpcRotate(direction, delta, selectedIndex);
        }
        ...
        private void SetLastIndex(int index)
        {
            ...
            if(_isServer) RpcSetLastIndex(index);
        }
        ...
        void RpcSetCalibrationType(CalibrationType calibrationType)
        {
            ...
            _infoDisplayInstance.SetText(calibrationType.ToString());
        }
        ...
        {
            PhysicalDisplayCalibration[] list = UnityEngine.Object.FindObjectsOfType<PhysicalDisplayCalibration>();
        ...
        void Update()
        {
            ...
            {
                CycleNextCalibrationType();
            }

            if(Input.GetKeyDown(KeyCode.KeypadPlus))
            {
                AdjustGridSelectSize(true);
            }

            if(Input.GetKeyDown(KeyCode.KeypadMinus))
            {
                AdjustGridSelectSize(false);
            }

            if(Input.GetKeyDown(KeyCode.Home))
            {
                InterpolateFromCorners();
            }
            ...
            if(Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift))
            {
                if(Input.GetKeyDown(KeyCode.Return))
                {
                    ...
                    if(index < 0)
                    {
                        selectedIndex = AllOptions.Count - 1;
                    }
                    else
                    {
                        selectedIndex = Mathf.Abs((selectedIndex - 1) % AllOptions.Count);
                    }
                    if(!noOptions)
                    {
                        DisplayShift();
            ...
            if(Input.GetKeyDown(KeyCode.A))
            {
                ...
                int lastIndex = vertexIndex - 1;
                vertexIndex = (lastIndex < 0) ? 63 : lastIndex;
            ...
            {
                vertexIndex = (vertexIndex + (64 - 8)) % 64;
                if(!noOptions)
                {
                    Debug.Log(vertexIndex);
                    VertexShift(direction, 1f);
            ...
            if(anyPressed)
            {
                Debug.Log("RealtimeCalibration: isServer = " + _isServer);
                if(_isServer)
                {
                    switch(_calibrationType)
                    {
                        case CalibrationType.Position:
                            PositionShift(direction, 0.0015f);
                            break;
                        case CalibrationType.Rotation:
                            RotationShift(direction, 0.10f);
                            break;
                        case CalibrationType.Vertex:
                            VertexShift(direction, 0.0015f);
                            break;
                    }
