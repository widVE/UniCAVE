...
namespace UniCAVE
{
    ...
    public class WindowsUtils
    {
        ...
        {
            WindowsUtils._setWidth = setWidth;
            WindowsUtils._setHeight = setHeight;
            WindowsUtils._desiredWidth = width;
            WindowsUtils._desiredHeight = height;
            WindowsUtils._desiredName = text;
            WindowsUtils._desiredX = posX;
            WindowsUtils._desiredY = posY;
            WindowsUtils._resolutions = WindowsUtils.loadAllWindowSizes();
        ...
        private static Dictionary<long, Vector2Int> loadAllWindowSizes()
        {
            ...
            WindowsUtils.EnumWindows(delegate (IntPtr wnd, IntPtr param)
            {
                RECT r = new RECT();
                WindowsUtils.GetWindowRect(wnd, out r);
...
#if UNITY_STANDALONE_WIN
            if(WindowsUtils._resolutions != null)
            {
                Dictionary<long, Vector2Int> newResolutions = WindowsUtils.loadAllWindowSizes();
                foreach(KeyValuePair<long, Vector2Int> kvp in newResolutions)
                {
                    if (kvp.Value.x != WindowsUtils._setWidth || kvp.Value.y != WindowsUtils._setHeight) continue;
                    WindowsUtils.SetWindowText(new IntPtr(kvp.Key), WindowsUtils._desiredName);
                    WindowsUtils.SetWindowPos(new IntPtr(kvp.Key), 0, WindowsUtils._desiredX, WindowsUtils._desiredY, WindowsUtils._desiredWidth, WindowsUtils._desiredHeight, WindowsUtils._desiredWidth * WindowsUtils._desiredHeight == 0 ? 1 : 0);
                    WindowsUtils._resolutions = null;
                    return true;
                }
            }
            else
